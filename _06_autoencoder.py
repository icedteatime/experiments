...
def structural_similarity(im1, im2):
    ...
    batch, channel, height, width = im1.shape
    image_size = channel*height*width

    im1 = im1.flatten(start_dim=1)
    im2 = im2.flatten(start_dim=1)

    ux = im1.mean(dim=-1)
    uy = im2.mean(dim=-1)
    cov = ((im1 - ux.unsqueeze(1)) * (im2 - uy.unsqueeze(1))).sum(dim=-1) / (image_size - 1)
    ...
    denominator = (ux**2 + uy**2 + c1) * (im1.var(dim=-1) + im2.var(dim=-1) + c2)
